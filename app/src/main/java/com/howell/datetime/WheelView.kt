...
class WheelView : View {
    ...
    private val maxTextLength: Int
        get() {
            ...
            for (i in Math.max(currentItem - addItems, 0) until Math.min(
                    currentItem + this.visibleItems, adapter.itemsCount)) {
    ...
    private fun initResourcesIfNecessary() {
        ...
        if (valuePaint == null) {
            valuePaint = TextPaint(Paint.ANTI_ALIAS_FLAG
                    or Paint.FAKE_BOLD_TEXT_FLAG or Paint.DITHER_FLAG)
    ...
    companion object {
        ...
        private val VALUE_TEXT_COLOR = -0x10000000

        /** Items text color  */
        private val ITEMS_TEXT_COLOR = -0x1000000
